  ...
  for name in os.listdir(dir):
    ...
    if os.path.isfile(path):
        ...
        try:
            ...
            print("Ошибка доступа к каталогу: ", path)
  return dict
...
def print_duplicates(dir, dict):
    ...
    for name_size_agr, cnt_name_size in dict_unique_files.items():
        if cnt_name_size > 1:
            print()
            print("Найдены дубликаты файла " + name_size_agr.split(";")[0] + " размером " + \
                  str(int(name_size_agr.split(";")[1])//1024+1) + " Кбайт, " + str(cnt_name_size) + " штук(и):")
            for path, name_size in dict.items():
                if name_size_agr == name_size:
                    print(path)
